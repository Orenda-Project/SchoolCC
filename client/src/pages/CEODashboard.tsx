...
export default function CEODashboard() {
  ...
  const handleExportSchool = (schoolId: string, format: 'sheets' | 'docs') => {
    ...
    // Prepare school data for export
    const exportData = {
      'School Name': school.name,
      'School ID': school.id,
      'Total Teachers': school.teachers,
      'Head Teachers': school.headTeachers,
      'Pending Requests': school.pendingRequests,
    ...
    if (format === 'sheets') {
      ...
      link.setAttribute('download', `${school.name.replace(/\s+/g, '_')}_data.csv`);
      link.style.visibility = 'hidden';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);

      console.log(`Exported ${school.name} to CSV for Google Sheets`);
    } else {
      // Export as formatted text for Google Docs
      const docContent = `SCHOOL DATA REPORT
===============================================

School Name: ${exportData['School Name']}
School ID: ${exportData['School ID']}

STAFF INFORMATION
-------------------
Total Teachers: ${exportData['Total Teachers']}
Head Teachers: ${exportData['Head Teachers']}

PERFORMANCE METRICS
-------------------
Pending Requests: ${exportData['Pending Requests']}
Compliance Score: ${exportData['Compliance Score']}

EXPORT DETAILS
-------------------
Export Date: ${exportData['Export Date']}
Export Time: ${exportData['Export Time']}

===============================================
Generated by SchoolCC Management System
`;
      ...
      link.setAttribute('href', url);
      link.setAttribute('download', `${school.name.replace(/\s+/g, '_')}_report.txt`);
      link.style.visibility = 'hidden';
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);

      console.log(`Exported ${school.name} to text file for Google Docs`);
    }
